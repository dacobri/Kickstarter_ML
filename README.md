...
│
├── streamlit_app/
│   ├── app.py                             # Streamlit prototype for campaign creators
│   ├── train_model.py                     # One-time script to train RF and save artifacts
│   ├── requirements.txt                   # App dependencies
│   └── artifacts/                         # Generated by train_model.py (model + lookup tables)
│
├── data/
...
An interactive web app that lets a Kickstarter creator enter their campaign details before launch and receive a predicted success probability, the key factors driving that prediction, and actionable advice on what to change.
...
cd streamlit_app
pip install -r requirements.txt
python train_model.py          # one-time setup (~30s) — trains RF model from notebook outputs
streamlit run app.py
```

The training script reads the parquet files produced by the notebooks and saves the trained model + all lookup tables to `streamlit_app/artifacts/`. The model file (113 MB) is not committed to the repo — `train_model.py` regenerates it.

### How It Works

The creator fills in goal amount, duration, video, blurb, title, launch timing, category, and country. The app replicates the exact 129-feature pipeline from notebooks 03b/04, runs the Random Forest model (Val AUC 0.787), and returns:

1. A success probability score with a colour-coded indicator
2. Which factors are helping or hurting the prediction
3. Specific advice for factors the creator can actually change (goal, video, duration, preparation time, blurb)
